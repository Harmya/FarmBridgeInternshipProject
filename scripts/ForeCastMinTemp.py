...
def monthly_fore():
    ...
    for x in range(180, 1128):
        ...
        if c == 13:
            ...
            w = min(sum_temp[0], sum_temp[1])
            s = min(sum_temp[2], sum_temp[3], sum_temp[4])
            s_w_m = min(sum_temp[5], sum_temp[6], sum_temp[7], sum_temp[8])
            n_e_m = min(sum_temp[9], sum_temp[10], sum_temp[11])
        ...
        c += 1
    FinalYearTmin.get_it(file_to_read, output_file)
